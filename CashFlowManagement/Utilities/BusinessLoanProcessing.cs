...
{
    public class BusinessLoanProcessing
    {
        public static List<LoanInterestTableViewModel> CalculatePaymentsByMonth(List<BusinessLoan> list, BusinessLoan loan, bool append)
        {
            list = list.OrderBy(x => x.StartDate).ToList();

            BusinessLoan parentLoan = list.Where(x => !x.ParentLoanId.HasValue).FirstOrDefault();
        ...
        public static double GetCurrentMonthlyPaymentByUser(string username)
        {
            ...
            {
                var loanList = business.BusinessLoan.Where(x => !x.DisabledDate.HasValue).ToList();
